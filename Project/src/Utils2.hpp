...
// Funzione che memorizza le celle 0D
PolygonalMesh calcolaCelle0D(vector<list<Vector3d>>& insiemePoligoni);

// Funzione che memorizza le celle1D e celle2D
void calcolaCelle1D2D(vector<list<Vector3d>>& insiemePoligoni, PolygonalMesh& Mesh);

// Funzione che stampa le informazioni della mesh (in tre file diversi: celle0D, celle1D, celle2D)
bool OutputPolygonalMesh(Struttura_DFN& DFN, const string& fileOutput);

// Funzione che controlla se punto in poligono
